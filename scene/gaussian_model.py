...
class GaussianModel:
    ...
    def save_ply(self, path):
        ...
        segments = self._segments.detach().cpu().numpy()
        ...
        # Create list of attributes including segment
        attributes_list = self.construct_list_of_attributes()
        attributes_list.append('segment')  # Add segment to the attributes list

        # Create dtype with all fields including segment
        dtype_full = [(attribute, 'f4') for attribute in attributes_list]

        elements = np.empty(xyz.shape[0], dtype=dtype_full)
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, scale, rotation, segments[:, None]), axis=1)
    ...
    def load_ply(self, path):
        ...
        if 'segment' in plydata.elements[0].properties:
            segments = np.asarray(plydata.elements[0]["segment"])
        else:
            segments = np.zeros(xyz.shape[0], dtype=np.int32)
        ...
        self._segments = torch.tensor(segments, dtype=torch.long, device="cuda")
    ...
    def prune_points(self, mask):
        ...
        self._segments = self._segments[valid_points_mask] 
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_segments):
        d = {"xyz": new_xyz,
        ...
        self._xyz = optimizable_tensors["xyz"]
        self._segments = torch.cat([self._segments, new_segments], dim=0)
        self._features_dc = optimizable_tensors["f_dc"]
        self._features_rest = optimizable_tensors["f_rest"]
        self._opacity = optimizable_tensors["opacity"]
        self._scaling = optimizable_tensors["scaling"]
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_segments = self._segments[selected_pts_mask].repeat(N)

        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, new_segments)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        new_segments = self._segments[selected_pts_mask]

        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_segments)
