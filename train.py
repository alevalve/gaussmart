...
def training(dataset, opt, pipe,
    ...
    # Lazy-init: only create when DINO actually starts
    dino_encoder = None
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # ------------------------------
        # DINO loss (starts after dino_start_iter)
        # ------------------------------
        if use_dino_loss and (iteration > dino_start_iter):
            if dino_encoder is None:
                dino_encoder = DINOImageEncoder()  # lazy init when needed
            # Optional: warmup (commented). To enable, uncomment and set W.
            # W = 1000
            # prog = min(1.0, (iteration - dino_start_iter) / W)
            # effective_lambda_dino = lambda_dino * prog
            effective_lambda_dino = lambda_dino
            d_loss = dino_loss(image, gt_image, dino_encoder, effective_lambda_dino)
        else:
        ...
        with torch.no_grad():
            ...
            try:
                with open(dino_loss_log_path, 'a', newline='') as csvfile:
                    writer = csv.DictWriter(csvfile, fieldnames=['iteration', 'dino_loss', 'total_loss', 'l1_loss', 'dist_loss', 'normal_loss'])
                    writer.writerow({
                        'iteration': iteration,
                        'dino_loss': float(d_loss.item()),
            ...
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end),
                            testing_iterations, scene, render, (pipe, background), lpips_loss)
...
if __name__ == "__main__":
    ...
    # NEW CLI: when DINO starts + weight
    ...
    training(lp.extract(args),
             ...
             dino_start_iter=args.dino_start_iter)
